...
if(!historicData){
  return <Alert message="No data available" type="Warning" />
    ...
    return(
      <div 
      className="flex flex-col item-center justify-center mt-6 p-6 w-full"
 ...
 let date = new Date(coinPrice[0]);// convertng unix timestamp to date
  ...
  datasets:[
    {
      label: `price (past ${days} ${days === 1 ? 'Day' : 'Days'}) in 
...
options={{
  responsive: true,
  maintainAspectRatio: false,// aloow us to resize the chart
  elements: {
    point: {
      radius: 0,
    }
         ...
         />
          ...
          <select className="select select-ghost w-full max-w-xs" onChange={handleDayChange}> 
